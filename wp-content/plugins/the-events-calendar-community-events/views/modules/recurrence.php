...
<tr class="recurrence-row">
	...
	<td>
		...
		<span id="recurrence-end" style="display: <?php echo !$recType || $recType == 'None' ? 'none' : 'inline' ?>">
			...
			<input autocomplete="off" placeholder="<?php echo TribeDateUtils::dateOnly( date( TribeDateUtils::DBDATEFORMAT ) ); ?>" type="text" id="recurrence_end" class="datepicker" name="recurrence[end]" value="<?php echo $recEnd; ?>" style="display:<?php echo !$recEndType || $recEndType == 'On' ? 'inline' : 'none'; ?>"/>

			<span id="rec-count" style="display:<?php echo $recEndType == 'After' ? 'inline' : 'none'; ?>">
				<input autocomplete="off" type="text" name="recurrence[end-count]" id="recurrence_end_count"  value="<?php echo $recEndCount ? $recEndCount : 1 ?>" style="width: 40px;"/>
